"""
load_stg.py — Chargement des fichiers plats hospitaliers vers STG (Snowflake)

À lancer en local uniquement : python dbt/load_stg.py [--date YYYYMMDD] [--data-dir CHEMIN]

Flux : fichiers .txt locaux → PUT (stage interne éphémère) → COPY INTO STG → REMOVE stage.
Sources : STG_DATA_DIR ou Inputs_Projets_NF26_AI07/Data Hospital sous la racine projet.
"""
...
def resolve_sql_dir() -> Path:
    ...
    for base in [cwd, *cwd.parents]:
        ...
        if (sql_dir / "snowflake_utils.py").is_file():
            return sql_dir
        if (base / "dbt" / "load_stg.py").is_file():
            return base / "SQL"
...
logger = setup_logger("load_stg", "load_stg.log")
...
# SELECT du COPY INTO (par position ; en-têtes HOSPITALISATION gérés via SKIP_HEADER)
...
def _require_local_execution() -> None:
    if _is_snowflake_workspace():
        raise RuntimeError(
            "load_stg.py doit être exécuté en local (python dbt/load_stg.py), "
            "pas depuis le Workspace Snowflake."
        )


def _project_data_dir() -> Path:
    return resolve_sql_dir().parent / "Inputs_Projets_NF26_AI07" / "Data Hospital"


def _default_data_dir() -> Path:
    """STG_DATA_DIR si défini, sinon dossier Inputs relatif au projet."""
...
def put_file(cursor, file_path: Path) -> None:
    ...
    # Le chemin doit être entre guillemets simples pour gérer les espaces
    cursor.execute(
        f"PUT 'file://{abs_path}' @{STAGE} AUTO_COMPRESS=FALSE OVERWRITE=TRUE"
    )
...
def load_stg_day(
    ...
    print(f"[load_stg] Démarrage — date={date}")
    print(f"[load_stg] Répertoire données : {resolved_data_dir}")
    print(f"[load_stg] Dossier source : {folder}")
    ...
    logger.info("=" * 60)
    print(f"[load_stg] FIN — {status} — durée {duration}")
    finalize_log()
    return success


def _ensure_file_handler() -> None:
    """Réouvre le handler fichier après une exécution précédente (notebook local)."""
    ...
    fh = WorkspaceFileHandler(LOG_DIR / "load_stg.log")
...
def finalize_log() -> None:
    ...
    if file_handler and not getattr(file_handler, "_closed", False):
        file_handler.close()
    verify_log_file("load_stg.log", "load_stg", handler=file_handler)


def run_load_stg(date: str, **kwargs) -> bool:
    """Raccourci : load_stg_day (exécution locale uniquement)."""
    return load_stg_day(date, **kwargs)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Charger les fichiers plats d'un jour dans STG (exécution locale)"
    )
    parser.add_argument(
        "--date",
        "-d",
    ...
    parser.add_argument(
        "--data-dir",
        help="Répertoire parent des dossiers BDD_HOSPITAL_* (fichiers .txt locaux)",
    )
    parser.add_argument(
        "--retention-days",
        type=int,
    ...
    if not args.date:
        print(f"[load_stg] Aucune date fournie — DEFAULT_LOAD_DATE={load_date}")
    if not args.data_dir:
        print(f"[load_stg] Aucun --data-dir — défaut={load_data_dir}")
